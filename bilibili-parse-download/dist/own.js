...
!function () {
        ...
        return cache_createClass(CacheFactory, null, [{
            ...
            value: function setValue() {
                var key = arguments.length > 0 && void 0 !== arguments[0] ? arguments[0] : "",
                    value = arguments.length > 1 ? arguments[1] : void 0, _key$split = key.split(".", 2),
                    _key$split2 = _slicedToArray(_key$split, 2), cacheName = _key$split2[0], cacheKey = _key$split2[1];
    ...
    var clazzMap = {}, VideoBase = function () {
        ...
        return video_base_createClass(Bangumi, [{
            ...
            value: function build() {
                ...
                var _step4, episodes = bangumiCache.get("episodes") || [], isEpMap = {},
                    _iterator4 = _createForOfIteratorHelper(episodes = [].concat(_toConsumableArray(episodes.filter((function (a) {
        ...
        return video_base_createClass(Cheese, [{
            ...
            key: "build",
            value: function build() {
                var epid, cheeseCache = cache.get("Cheese"),
                    sid = (location.href.match(/\/cheese\/play\/ss(\d+)/i) || ["", ""])[1];
        ...
        get_quality: function get_quality() {
            ...
            if ("cheese" === _type) {
                var q = $("div.edu-player-quality-item.active span").text(),
                    q_max = $($("div.edu-player-quality-item span").get(0)).text();
                _q = q in q_map ? q_map[q] : 0, _q_max = q_max in q_map ? q_map[q_max] : 0;
            } else {
                var keys = Object.keys(videoQualityMap),
                    _q2 = parseInt(("video" === _type ? $("li.bpx-player-ctrl-quality-menu-item.bpx-state-active") : $("li.squirtle-select-item.active")).attr("data-value")),
                    _q_max2 = parseInt($(("video" === _type ? $("li.bpx-player-ctrl-quality-menu-item") : $("li.squirtle-select-item")).get(0)).attr("data-value"));
    ...
    function get_url_base(page, quality, video_format, success, error, request_type) {
        ...
        var vb = video.base(),
            _ref = [vb.aid(page), vb.bvid(page), vb.cid(page), vb.epid(page), quality || video.get_quality().q, vb.type()],
            aid = _ref[0], bvid = _ref[1], cid = _ref[2], epid = _ref[3], q = _ref[4], type = _ref[5],
            format = video_format || config_config.format;
    ...
    function _get_subtitle(p, callback) {
        var to_blob_url = !(arguments.length > 2 && void 0 !== arguments[2]) || arguments[2], vb = video.base(),
            _ref4 = [vb.aid(p), vb.cid(p), vb.epid(p)], aid = _ref4[0], cid = _ref4[1], epid = _ref4[2];
        ajax({
                ...
                try {
                    for (_iterator2.s(); !(_step2 = _iterator2.n()).done;) {
                        var data = _step2.value,
                            a = new Date(1e3 * (parseInt(data.from) - 28800)).toTimeString().split(" ")[0] + "." + (data.from.toString().split(".")[1] || "000").padEnd(3, "0"),
                            b = new Date(1e3 * (parseInt(data.to) - 28800)).toTimeString().split(" ")[0] + "." + (data.to.toString().split(".")[1] || "000").padEnd(3, "0");
    ...
    var JSZip, DPlayer, QRCode, md5, count = 0, scripts = [], getModules = [],
        initIframe = function initIframe(name, ver, filename, getModule) {
            count++, new RuntimeLib({
                ...
                0 == --count && (!function iframeInvoke(scripts, getModules) {
                    ...
                    var scriptTags = scripts.map((function (code) {
                        ...
                        html = '<!DOCTYPE html><html><head><meta charset="utf-8"><title>Runtime Library</title></head><body>'.concat(scriptTags, "</body></html>"),
                        blobUrl = URL.createObjectURL(new Blob([html], {
                            type: "text/html"
        ...
        if (download_rpc_clicked) message_Message_miaow(); else {
            ...
            ajax(function get_rpc_post(data) {
                ...
                if (rpc.port) {
                    ...
                    url = `${rpc.domain}/jsonrpc`;
                }

                return {
                    url: url,//"".concat(rpc.domain, ":").concat(rpc.port, "/jsonrpc"),
                    type: "POST",
                    dataType: "json",
    ...
    function open_ariang(rpc) {
        var hash_tag = rpc ? "#!/settings/rpc/set/".concat(rpc.domain.replace("://", "/"), "/").concat(rpc.port, "/jsonrpc/").concat(window.btoa(rpc.token)) : "",
            url = config_config.ariang_host + hash_tag, a = document.createElement("a");
    ...
    function download_rpc_ariang_send(video) {
        ...
        bp_aria2_window && !bp_aria2_window.closed || (open_ariang(), time = 3e3), setTimeout((function () {
            var bp_aria2_window = window.bp_aria2_window,
                task_hash = "#!/new/task?" + ["url=".concat(encodeURIComponent(window.btoa(video.url))), "out=".concat(encodeURIComponent(video.filename)), "header=User-Agent:".concat(window.navigator.userAgent), "header=Referer:".concat(window.location.href)].join("&");
    ...
    function _download_danmaku_ass(cid, title) {
        var return_type = arguments.length > 2 && void 0 !== arguments[2] ? arguments[2] : null,
            callback = arguments.length > 3 && void 0 !== arguments[3] ? arguments[3] : null;
        ajax({
            url: "https://api.bilibili.com/x/v1/dm/list.so?oid=".concat(cid),
            dataType: "text"
        }).then((function (result) {
            ...
            var _step2, danmaku_data = result_dom.find("d").map((function (i, el) {
                    ...
                    var text = danmaku.text, time = danmaku.time,
                        commands = [0 === danmaku.type ? function scrollCommand(top, left_a, left_b) {
                            ...
                            var div = function div(i, j) {
                                    ...
                                    return n < 10 ? "0" + n : "" + n;
                                }, integer = Math.floor(seconds), hour = div(integer, 3600), minute = div(integer, 60) % 60,
                                second = integer % 60, minorSecond = Math.floor(100 * (seconds - integer));
                ...
                fix_id = 0, _iterator2 = download_createForOfIteratorHelper(danmaku_data);
            try {
                for (_iterator2.s(); !(_step2 = _iterator2.n()).done;) {
    ...
    function download_subtitle_vtt() {
        var p = arguments.length > 0 && void 0 !== arguments[0] ? arguments[0] : 0,
            file_name = arguments.length > 1 ? arguments[1] : void 0,
            download_subtitle = function download_subtitle(blob_url) {
                if (blob_url) {
    ...
    var _document$head$innerH, Download = {
            ...
            download: function download(url, filename, type) {
                console.log("Download.download", url, filename, arguments)

                filename = filename.replace(/[\/\\*|]+/g, "-").replace(/:/g, "：").replace(/\?/g, "？").replace(/"/g, "'").replace(/</g, "《").replace(/>/g, "》"),
                    "blob" === type ? download_blob(url, filename) : ("rpc" === type || "post" === type) && function download_rpc(url, filename) {
                        console.log("arguments", arguments)
                        var type = arguments.length > 2 && void 0 !== arguments[2] ? arguments[2] : "post";
                        console.log("Download.download_rpc", url, filename, type)
            ...
            download_all: function download_all() {
                ...
                $("body").on("click", 'input[name="option_video"]', (function (event) {
                    ...
                    event.ctrlKey || event.altKey) {
                        var current_select_option_index = get_option_index(event.target),
                            option_videos = download_toConsumableArray(document.getElementsByName("option_video"));
                ...
                var _step, option_support_html = "",
                    _iterator = download_createForOfIteratorHelper(video.get_quality_support());
                try {
                    for (_iterator.s(); !(_step = _iterator.n()).done;) {
                ...
                function download_videos(video_tasks, i, videos) {
                    if (video_tasks.length) {
                        ...
                        var task = video_tasks[i],
                            msg = "第".concat(i + 1, "（").concat(i + 1, "/").concat(video_tasks.length, "）个视频");
                        ...
                        api.get_urls(task.p, task.q, task.format, (function success(res) {
                            if (setTimeout((function () {
                                ...
                                var _ref3 = [res.url, rpc_type(), res.video, res.audio], url = _ref3[0], type = _ref3[1],
                                    video_url = _ref3[2], audio_url = _ref3[3];
    ...
    var Auth = function () {
            ...
            return function auth_createClass(Constructor, protoProps, staticProps) {
                ...
                value: function checkLoginStatus() {
                    var _this = this,
                        _ref = [store.get("auth_id"), store.get("auth_sec"), store.get("access_key"), store.get("auth_time") || 0],
                        auth_id = _ref[0], auth_sec = _ref[1], access_key = _ref[2], auth_time = _ref[3];
                    if (auth_id || auth_sec) {
                ...
                value: function login() {
                    var useApp = arguments.length > 0 && void 0 !== arguments[0] ? arguments[0] : "1",
                        do_login = "1" === useApp ? this.loginApp.bind(this) : this.loginWeb.bind(this);
                ...
                value: function loginApp() {
                    ...
                    this._login((function (res) {
                        if (res && !res.code) {
                            var _res$data = res.data, url = _res$data.url, auth_code = _res$data.auth_code, is_login = 0,
                                box = MessageBox_alert('<p>请使用<a href="https://app.bilibili.com/" target="_blank">哔哩哔哩客户端</a>扫码登录</p><div id="login_qrcode"></div>', (function () {
        ...
        left.children().remove(), right.children().remove(), Object.keys(btn_list).map((function (key) {
            if ("more" !== key) {
                ...
                var svg = svg_map[key].replaceAll("#757575", "currentColor").replace("class", 'class="'.concat(item.find("svg").attr("class"), '"')),
                    span = item.find("span").text(btn_list[key]), item_div = item.find("div").eq(0);
    ...
    function initToolbar() {
        if ($("#arc_toolbar_report")[0]) showVideoToolbar("arc_toolbar_report"); else if ($("#playlistToolbar")[0]) showVideoToolbar("playlistToolbar"); else if ($("#videoToolbar")[0]) !function showFestivalToolbar(toolbar_id) {
            ...
            var left = toolbar_obj_2.find(".video-toolbar-content_left"),
                right = toolbar_obj_2.find(".video-toolbar-content_right");
                ...
                Object.keys(btn_list).map((function (key) {
                    if ("more" !== key) {
                        ...
                        var svg = svg_map[key].replaceAll("#757575", "currentColor"),
                            item_icon = item.find(".content-item_icon").eq(0);
            ...
            var toolbar_obj = $(".".concat(toolbar_class)).eq(0), toolbar_obj_2 = toolbar_obj.clone(),
                left = toolbar_obj_2.find(".toolbar-left"), right = toolbar_obj_2.find(".toolbar-right");
            left.children().remove(), right.children().remove(), Object.keys(btn_list).map((function (key) {
                if ("more" !== key) {
                    var item = toolbar_obj.find(".toolbar-left").children().eq(0).clone();
                    item.attr("id", key), item.attr("title", btn_list[key]);
                    var svg = svg_map[key].replaceAll("#757575", "currentColor").replace("class", 'class="'.concat(item.find("svg").attr("class"), '"')),
                        span = item.find("span").text(btn_list[key]);
            ...
            var toolbar_obj = $(".edu-play-left").children().eq(1), toolbar_class = toolbar_obj.attr("class"),
                span_class = toolbar_obj.children().eq(0).attr("class"),
                span_class_svg = toolbar_obj.children().eq(0).children().eq(0).attr("class"),
                span_class_text = toolbar_obj.children().eq(0).children().eq(1).attr("class");
            ...
            value: function run() {
                ...
                var evt = {
                    ...
                    bilibili_parse: function bilibili_parse() {
                        user.lazyInit(!0);
                        var vb = video.base(), _ref = [vb.type(), vb.aid(), vb.p(), vb.cid(), vb.epid()],
                            type = _ref[0], aid = _ref[1], p = _ref[2], cid = _ref[3], epid = _ref[4],
                            q = video.get_quality().q;
                        ...
                        var _ref2 = [store.get("auth_id"), store.get("auth_sec")], auth_id = _ref2[0],
                            auth_sec = _ref2[1];
                    ...
                    video_download: function video_download() {
                        ...
                        console.log("video_download:", type)
                        if ("web" === type) $("#video_url")[0].click(); else if ("a" === type) {
                            var _ref3 = [$("#video_url").attr("href"), $("#video_url_2").attr("href"), $("#video_url").attr("download"), $("#video_url_2").attr("download")],
                                video_url_2 = _ref3[1], file_name = _ref3[2], file_name_2 = _ref3[3],
                                msg = "建议使用IDM、FDM等软件安装其浏览器插件后，鼠标右键点击链接下载~<br/><br/>" + '<a href="'.concat(_ref3[0], '" download="').concat(file_name, '" target="_blank" style="text-decoration:underline;">&gt视频地址&lt</a><br/><br/>') + ("dash" === config_config.format ? '<a href="'.concat(video_url_2, '" download="').concat(file_name_2, '" target="_blank" style="text-decoration:underline;">&gt音频地址&lt</a>') : "");
                            MessageBox_alert(msg);
                        } else if ("aria" === type) {
                            var _ref4 = [$("#video_url").attr("href"), $("#video_url_2").attr("href")],
                                _video_url = _ref4[0], _video_url_ = _ref4[1], video_title = video.base().filename(),
                                _file_name = video_title + Download.url_format(_video_url),
                                _file_name_ = video_title + ".m4a",
                                aria2c_header = '--header "User-Agent: '.concat(window.navigator.userAgent, '" --header "Referer: ').concat(window.location.href, '"'),
                                _ref6 = main_slicedToArray({
                                    min: [1, 5],
                                    mid: [16, 8],
                                    max: [32, 16]
                                }[config_config.aria2c_connection_level] || [1, 5], 2), url_max_connection = _ref6[0],
                                server_max_connection = _ref6[1],
                                aria2c_max_connection_parameters = "--max-concurrent-downloads ".concat(url_max_connection, " --max-connection-per-server ").concat(server_max_connection),
                                _map = ['aria2c "'.concat(_video_url, '" --out "').concat(_file_name, '"'), 'aria2c "'.concat(_video_url_, '" --out "').concat(_file_name_, '"')].map((function (code) {
                                ...
                                })), _map2 = main_slicedToArray(_map, 2), code = _map2[0], code_2 = _map2[1],
                                _msg = "点击文本框即可复制下载命令！<br/><br/>" + '视频：<br/><input id="aria2_code" value=\''.concat(code, '\' onclick="bp_clip_btn(\'aria2_code\')" style="width:100%;"></br></br>') + ("dash" === config_config.format ? '音频：<br/><input id="aria2_code_2" value=\''.concat(code_2, '\' onclick="bp_clip_btn(\'aria2_code_2\')" style="width:100%;"><br/><br/>') + '全部：<br/><textarea id="aria2_code_all" onclick="bp_clip_btn(\'aria2_code_all\')" style="min-width:100%;max-width:100%;min-height:100px;max-height:100px;">'.concat(code, "\n").concat(code_2, "</textarea>") : "");
