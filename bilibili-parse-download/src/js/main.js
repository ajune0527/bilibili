...
class Main {
    ...
    init() {
        ...
        $(`#${root_div.id}`).append(
            '<a id="video_url" style="display:none;" target="_blank" href="#"></a>'
        );
        $(`#${root_div.id}`).append(
            '<a id="video_url_2" style="display:none;" target="_blank" href="#"></a>'
        );
    }
        ...
        const evt = {
            ...
            bilibili_parse() {
                ...
                api.get_url((res) => {
                    if (res && !res.code) {
                        ...
                        $('#video_url').attr(
                            'download',
                            vb.filename() + Download.url_format(url)
                        );
                        $('#video_download').show();
                        if (url_2 !== '#') {
            ...
            video_download_all() {
                ...
                if (auth.hasAuth()) {
                    if (config.download_type === 'rpc') {
                        ...
                        MessageBox.confirm(
                            '仅支持使用RPC接口批量下载，请确保RPC环境正常，是否继续？',
                            () => {
                                Download.download_all();
                            }
                        );
            ...
            video_download() {
                ...
                if (type === 'web') {
                    ...
                    const msg =
                        '建议使用IDM、FDM等软件安装其浏览器插件后，鼠标右键点击链接下载~<br/><br/>' +
                        `<a href="${video_url}" download="${file_name}" target="_blank" style="text-decoration:underline;">&gt视频地址&lt</a><br/><br/>` +
                        (config.format === 'dash'
                            ? `<a href="${video_url_2}" download="${file_name_2}" target="_blank" style="text-decoration:underline;">&gt音频地址&lt</a>`
                            : '');
                    ...
                    const [code, code_2] = [
                        ...
                        (code) =>
                            `${code} ${aria2c_header} ${aria2c_max_connection_parameters} ${config.aria2c_addition_parameters}`
                    );
                    const msg =
                        '点击文本框即可复制下载命令！<br/><br/>' +
                        `视频：<br/><input id="aria2_code" value='${code}' onclick="bp_clip_btn('aria2_code')" style="width:100%;"></br></br>` +
                        (config.format === 'dash'
                            ? `音频：<br/><input id="aria2_code_2" value='${code_2}' onclick="bp_clip_btn('aria2_code_2')" style="width:100%;"><br/><br/>` +
                    ...
                    const url = $('#video_url').attr('href');
                    const filename =
                        video.base().filename() + Download.url_format(url);
