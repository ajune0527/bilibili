...
function make_toolbar_bangumi(main_class_name, sub_class_names) {
    ...
    const more_element = (id, name) =>
        `<li><span id="${id}">${name}</span></li>`;

    let toolbar_elements = Object.keys(btn_list)
        .map((key) => {
            if (key === 'more') {
                ...
                return (
                    ...
                    <ul>${Object.keys(more_map)
                        .map((key) => more_element(key, more_map[key]))
                        .join('')}</ul>
                </div>`
                );
            }
            ...
            const el =
                ...
                `<div class="more">更多<div class="more-ops-list">
                    <ul>${Object.keys(more_map)
                        .map(
                            (key) =>
                                `<li><span id="${key}">${more_map[key]}</span></li>`
                        )
                        .join('')}</ul>
                </div>`;
...
function showFestivalToolbar(toolbar_id) {
    ...
    Object.keys(btn_list).map((key) => {
        ...
        const item = toolbar_obj
            .find('.video-toolbar-content_item')
            .eq(0)
            .clone();
        item.attr('id', key);
...
function showVideoCardToolbar(toolbar_class) {
    ...
    toolbar_obj.each(function () {
        ...
        if (match) {
            ...
            const $i = $(
                '<i class="vui_icon sic-BDC-arrow_up_triangle_fill " style="transform: rotate(180deg);margin-left: 5px;"></i>'
            );
            // 创建新 span 元素
...
function initToolbar() {
    if (!!$('.bili-video-card__details')[0]) {
        ...
        const span_class_svg = toolbar_obj
            .children()
            .eq(0)
            .children()
            .eq(0)
            .attr('class');
        const span_class_text = toolbar_obj
            .children()
            .eq(0)
            .children()
            .eq(1)
            .attr('class');
        toolbar_obj.after(
            make_toolbar_bangumi(toolbar_class, [
                span_class,
                span_class_svg,
                span_class_text,
            ])
